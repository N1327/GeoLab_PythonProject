...
def init_db():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(email='admin@example.com').first():
            admin = User(name='admin', email='admin@example.com', password='12345678', is_admin=True)
            db.session.add(admin)
            print("Admin user added.")
        for sport, posts_list in posts_data.items():
            for p in posts_list:
                if not Post.query.get(p['id']):
                    post = Post(
                        id=p['id'],
                        sport=sport,
                        title=p['title'],
                    ...
                    db.session.add(post)
                    print(f"Added post: {p['title']}")

        db.session.commit()
        print("Database initialized with posts and admin user.")
